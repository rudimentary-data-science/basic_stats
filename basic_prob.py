...
def equal(col: list[float], val: Any) -> list[bool]:
    """Returns a boolean list that has true if the corresponding list element
    is equal to the given number"""
    return [x == val for x in col]


def less(col: list[float], val: Any) -> list[bool]:
    """Returns a boolean list that has true if
    the corresponding list element is lesser than the given number"""
    return [x < val for x in col]


def aand(col1: list[bool], col2: list[bool]) -> list[bool]:
    """Returns a boolean list by performing logical
    and between the 2 list elements"""
...
def oor(col1: list[bool], col2: list[bool]) -> list[bool]:
    """ Returns a boolean list by performing logical
    or between the 2 list elements"""
    if isBoolean(col1) and isBoolean(col2):
        if len(col1) == len(col2):
    ...
    if type(col) == list:
        ...
        if all(x <= 1 for x in prob1.values()) and all(x <= 1 for x in col.values()):
            if sum(col.values()) == 1:
                ...
                raise ValueError(
                    "Sum of probability values of the 2nd argument should be equal to 1")
        else:
            raise ValueError("Probability values should be less than 1")
...
class discrete_rv:
    ...
    def transform(self, f):
        ...
        a2 = {val: sum([self.probs[i] for i, j in a1.items() if j == val])
              for val in set(a1.values())}
...
class Binomial_rv(discrete_rv):
    ...
    def __init__(self, p, n):
        ...
        if type(p) == float or type(p) == int:
            if 0 <= p <= 1:
                self.p = p
                self.probs = {i: (factorial(self.n) / (factorial(self.n - i) * factorial(i))) * (p ** i) * (
                    (1 - p) ** (self.n - i)) for i in range(n + 1)}
            else:
                raise ValueError("Expected: 0<=p<=1")
        elif type(p) == dict:
            if len(p) == self.n:
                ...
                raise ValueError(
                    "Incompatible dictionary size, expected length n ({})".format(self.n))
        else:
            raise TypeError("Expected p to be a number")
